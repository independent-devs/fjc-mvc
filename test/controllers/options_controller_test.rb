...
class OptionsControllerTest < ActionDispatch::IntegrationTest
  ...
  test 'should create option' do
    assert_difference('Option.count') do
      post options_url, params: { option: { name: @option.name, option_type_id: @option.option_type_id } }
    end
  ...
  test 'should update option' do
    patch option_url(@option), params: { option: { name: @option.name, option_type_id: @option.option_type_id } }
